...
public class LoadConfigs
{
    ...
    public void LoadAllBosses()
    {
        if (eb.SavedData.getCustomConfig().contains("Bosses"))
        {
            ...
            {
                for (String s : eb.SavedData.getCustomConfig().getStringList(
                        "Bosses"))
                {
                    ...
                    if (getLoadBoss(Splits[0]) != null)
                    {
                        ...
                        Location l = new Location(Bukkit.getWorld(Splits[2]),
                                Integer.parseInt(Splits[3]),
                                Integer.parseInt(Splits[4]),
                                Integer.parseInt(Splits[5]));

                        Boss bs = new Boss(lb.getName(), lb.getHealth(), l,
                                lb.getType(), lb.getDamage(), lb.getShowhp(),
                                lb.getItems(), lb.getSkills());
    ...
    public void LoadBosses()
    {
        ...
        {
            for (String name : eb.Bosses.getCustomConfig()
                    .getConfigurationSection("Bosses").getKeys(false))
            {
                String Name, Type;
                int Health, Damage;
                ...
                Name = name;
                Type = eb.Bosses.getCustomConfig().getString(
                        "Bosses." + name + ".Type");
                Health = eb.Bosses.getCustomConfig().getInt(
                        "Bosses." + name + ".Health");
                Damage = eb.Bosses.getCustomConfig().getInt(
                        "Bosses." + name + ".Damage");
                if (eb.Bosses.getCustomConfig().contains(
                        "Bosses." + name + ".Drops") == true)
                {
                    Items = eb.Bosses.getCustomConfig().getStringList(
                            "Bosses." + name + ".Drops");
                }
                if (eb.Bosses.getCustomConfig().contains(
                        "Bosses." + name + ".Skills") == true)
                {
                    Skills = eb.Bosses.getCustomConfig().getStringList(
                            "Bosses." + name + ".Skills");
                }
                showhp = eb.Bosses.getCustomConfig().getBoolean(
                        "Bosses." + name + ".Showhp");
    ...
    public void SaveAllBosses()
    {
        if (eb.BossList != null)
        {
            ...
            for (Boss boss : eb.BossList)
            {
                ...
                {
                    String save = boss.getName() + ":" + boss.getHealth() + ":"
                            + boss.getWorkingLocation().getWorld().getName()
                            + ":" + boss.getWorkingLocation().getBlockX() + ":"
                            + boss.getWorkingLocation().getBlockY() + ":"
                            + boss.getWorkingLocation().getBlockZ() + ":"
                            + boss.getTimer();
                    saved.add(save);
                }
