...
public class DamageListener implements Listener
{
        ...
        if (event.getEntity() != null)
        {
            if (eb.bossCalculator.isBoss(event.getEntity()))
            {
                if (!(event.getCause() == DamageCause.ENTITY_ATTACK
                        || event.getCause() == DamageCause.PROJECTILE || event
                            .getCause() == DamageCause.MAGIC))
                {
    ...
    public void Damage(EntityDamageByEntityEvent e)
    {
        ...
        if (Damager instanceof LivingEntity && Hited instanceof LivingEntity)
        {
            ...
            LivingEntity hited = (LivingEntity) Hited; // Hited entity is now
                                                       // "hited" since it's a
                                                       // LivingEntity
                ...
                if (!eb.bossCalculator.BossHited(hited))
                {
                    ...
                    else if (boss.getShowHp())
                    {
                        if (Damager instanceof Player)
                        {
                            ...
                            {
                                String s = eb.name + ChatColor.RED
                                        + boss.getName() + ChatColor.GRAY
                                        + " [" + ChatColor.DARK_RED
                                        + boss.getHealth() + ChatColor.GRAY
                                        + "/" + ChatColor.DARK_RED
                                        + boss.getMaxHealth() + ChatColor.GRAY
                                        + "]";
                                p.sendMessage(s);
                            }
                            else
                            {
                                double per = (((double) boss.getHealth() / boss
                                        .getMaxHealth()) * 10) + 1;
                                if (!boss.hasPercent((int) per))
                                {
                                    String s = eb.name + ChatColor.RED
                                            ...
                                            + ((int) per * 10) + "%"
                                            + ChatColor.GRAY + "]";
                                    for (Player player : eb.skillhandler
                                            .getPlayersRadious(20, boss))
                                    {
