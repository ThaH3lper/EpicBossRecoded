					...
					if(list.get(0).equals(ChatColor.DARK_PURPLE +"" + ChatColor.ITALIC + "A very mysterious egg"))
					{
						...
						if(lb != null)
						{
					        ...
					        eb.getServer().getScheduler().scheduleSyncDelayedTask(eb, new Runnable() {
					        	   ...
					        	   public void run() {
					        		   eb.BossList.add(new Boss(lb.getName(), lb.getHealth(), item.getLocation(), lb.getType(), lb.getDamage(), lb.getShowhp(), lb.getItems(), lb.getSkills()).showTitle(lb.getShowtitle()).setSkinUrl(lb.getSkin()));
		...
		if(e.getItem().getTypeId() == 383)
		{
			if(e.getItem().getItemMeta().hasLore())
			{
				...
				if(list.get(0).equals(ChatColor.DARK_PURPLE +"" + ChatColor.ITALIC + "A very mysterious egg"))
				{
					...
					if(lb != null)
					{
						Boss b = new Boss(lb.getName(), lb.getHealth(), e.getBlock().getLocation(), lb.getType(), lb.getDamage(), lb.getShowhp(), lb.getItems(), lb.getSkills()).showTitle(lb.getShowtitle()).setSkinUrl(lb.getSkin());
						eb.BossList.add(b);			
						eb.timer.despawn.DeSpawnEvent(eb);
