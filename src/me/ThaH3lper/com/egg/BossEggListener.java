					...
					if(list.get(0).equals(ChatColor.DARK_PURPLE +"" + ChatColor.ITALIC + "A very mysterious egg"))
					{
						...
						if(lb != null)
						{
					        ...
					        eb.getServer().getScheduler().scheduleSyncDelayedTask(eb, new Runnable() {
					        	   ...
					        	   public void run() {
					        		   eb.BossList.add(new Boss(lb.getName(), lb.getHealth(), item.getLocation(), lb.getType(), lb.getDamage(), lb.getShowhp(), lb.getItems(), lb.getSkills(), lb.getShowtitle(), lb.getSkin()));
